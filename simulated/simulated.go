...
func NewSimulatedBackendWithDatabase(database ethdb.Database, alloc types.GenesisAlloc, gasLimit uint64, concreteRegistry concrete.PrecompileRegistry) *SimulatedBackend {
	...
	blockchain.SetConcrete(concreteRegistry)
...
func (b *SimulatedBackend) rollback(parent *types.Block) {
	blocks, _ := core.GenerateChainWithConcrete(b.blockchain.Config(), parent, ethash.NewFaker(), b.database, 1, b.blockchain.Concrete(), func(int, *core.BlockGen) {})
...
func (b *SimulatedBackend) TransactionReceipt(ctx context.Context, txHash common.Hash) (*types.Receipt, error) {
	...
	receipt, _, _, _ := rawdb.ReadReceipt(b.database, txHash, b.blockchain.Config())
	if receipt == nil {
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call ethereum.CallMsg) (uint64, error) {
	...
	if call.Gas >= params.TxGas {
		...
		gasCap = b.pendingBlock.GasLimit()
	}

	hi, _, err := gasestimator.Estimate(ctx, msg, opts, gasCap)
	return hi, err
}
...
func (b *SimulatedBackend) callContract(ctx context.Context, call ethereum.CallMsg, header *types.Header, stateDB *state.StateDB) (*core.ExecutionResult, error) {
	...
	stateDB.SetBalance(call.From, uint256.MustFromBig(math.MaxBig256))
...
func (b *SimulatedBackend) SendTransaction(ctx context.Context, tx *types.Transaction) error {
	...
	// Include tx in chain
	blocks, receipts := core.GenerateChainWithConcrete(b.blockchain.Config(), block, ethash.NewFaker(), b.database, 1, b.blockchain.Concrete(), func(number int, block *core.BlockGen) {
...
func (b *SimulatedBackend) AdjustTime(adjustment time.Duration) error {
	...
	blocks, _ := core.GenerateChainWithConcrete(b.blockchain.Config(), block, ethash.NewFaker(), b.database, 1, b.blockchain.Concrete(), func(number int, block *core.BlockGen) {
